...
extract($_GET);
$medicine = get_one("select * from tbl_services where id = $id");
     ...
     <div class="row">
                                    <div class="col-md-12">
                                        ...
                                        <div class="form-row">
                                            <div class="form-group col-md-12">
                                                <label>Name:</label>
                                                <input type="text" name="name" class="form-control text-uppercase" value="<?= $medicine->name ?>">
                                            </div>
                                        </div>
                                        ...
                                        <div class="form-row">
                                            <div class="form-group col-md-12">
                                                ...
                                                <textarea name="description" class="form-control" id="" cols="3" rows="3"><?= $medicine->description ?></textarea>
                                            </div>
                                        </div>
                                        <div class="form-row">
                                            <div class="form-group col-md-12">
                                                <label>Price:</label>
                                                <input type="number" name="price" class="form-control text-uppercase" value="<?= $medicine->price ?>">
                                            </div>
                                        </div>
...
<div class="card shadow mb-4">
    ...
    <div class="card-body">
        ...
        <table class="table table-bordered" width="100%" cellspacing="0" id="tbl_equipment">
          ...
          <tbody>
            <?php foreach (get_all("select ms.* from tbl_medicine_stock ms where ms.medicine_id = $id") as $row) { ?>
              <tr>
                <td><?= $row['id'] ?></td>
                <td><?= $medicine->name ?></td>
                ...
                <td style="d-flex">
                    <a href="view_medicine_batch.php?id=<?= $row['id'] ?>" target="_blank" class="btn btn-primary">View</a>
                    <button type="submit" name="delete_medicine_batch" value="<?= $row['id'] ?>" class="btn btn-danger">Delete</button></td>
              </tr>
            <?php } ?>
                            ...
                            <div class="d-flex">
                                <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#medicineModal"  > Add Batch</button>
                                <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#medicineModalStock" style="margin-left:5px" > Add Stock</button>
