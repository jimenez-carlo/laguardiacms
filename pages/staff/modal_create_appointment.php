              ...
              <div class="col-md-12">
                  ...
                  <div class="form-group col-md-12">
                    ...
                    <select name="patient_id" id="patient_id" class="select2 form-control" required style="width:100%">
                      <?php foreach (mysqli_query($conn, "SELECT * from patient") as $row) { ?>
                        <option value="<?= $row['id'] ?>" <?= (isset($_POST['patient_id']) && $row['id'] == $_POST['patient_id']) ? 'selected' : '' ?>><?= strtoupper($row['fname'].' '.$row['mname'].' '.$row['lname']) ?></option>
                      <?php } ?>
                    </select>
                  </div>
                </div>
                  ...
                  <div class="form-group col-md-12">
                    ...
                    <select name="doctor_id" id="doctor_id" class="select2 form-control" required style="width:100%">
                      <?php foreach (mysqli_query($conn, "SELECT * from doctor") as $row) { ?>
                        <option value="<?= $row['id'] ?>" <?= (isset($_POST['doctor_id']) && $row['id'] == $_POST['doctor_id']) ? 'selected' : '' ?>><?= strtoupper($row['fname'].' '.$row['mname'].' '.$row['lname']) ?></option>
                      <?php } ?>
                    </select>
                  </div>
                ...
                <div class="form-row">
                  <div class="form-group col-md-12">
                    ...
                    <input type="date" name="appointment_date" class="form-control" id="recipient-name" required value="<?= isset($_POST['appointment_date']) ? $_POST['appointment_date'] : date('Y-m-d') ?>">
                  </div>
                </div>
              </div>
            </div>
