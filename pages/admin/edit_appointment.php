                                                ...
                                                <div class="d-flex float-right" style="width:fit-content">
                                                    <div class="input-group mb-2">
                                                        <div class="input-group-prepend">
                                                            <button type="submit" class="form-control btn btn-primary" <?= in_array($appointment->status,['rejected','closed','completed']) ? 'disabled' : '' ?> name="change_status"> Change Status</button>
                                                        </div>
                                    ...
                                    <div class="col-md-12">
                                            ...
                                            <div class="form-group col-md-4">
                                                ...
                                                <select name="patient_id" id="patient_id" class="form-control selectn">
                                                    <?php foreach (mysqli_query($conn, "SELECT * from patient") as $row) { ?>
                                                        <option value="<?= $row['id'] ?>" <?= $appointment->patient_id == $row['id'] ? 'selected' : '' ?>><?= strtoupper($row['fname'].' '.$row['mname'].' '.$row['lname']) ?></option>
                                                    <?php } ?>
                                                </select>
                                            </div>
                                            <div class="form-group col-md-4">
                                                ...
                                                <select name="doctor_id" id="doctor_id" class="form-control selectn">
                                                    <?php foreach (mysqli_query($conn, "SELECT * from doctor") as $row) { ?>
                                                        <option value="<?= $row['id'] ?>" <?= $appointment->doctor_id == $row['id'] ? 'selected' : '' ?>><?= strtoupper($row['fname'].' '.$row['mname'].' '.$row['lname']) ?></option>
                                                    <?php } ?>
                                                </select>
                                            </div>
                                            <div class="form-group col-md-4">
                                                <label>Appointment Date:</label>
                                                <input type="date" name="appointment_date" value="<?= $appointment->appointment_date ?>" class="form-control" id="recipient-name">
                                            </div>
                                        </div>
                                        ...
                                        <div class="form-row">
                                            <div class="form-group col-md-12">
                                                <label>Remarks:</label>
                                                <textarea name="remarks" class="form-control" id="" cols="3" rows="3"><?= $appointment->remarks ?></textarea>
                                            </div>
                                        </div>
    ...
    <div class="card-body">
        ...
        <table class="table table-bordered" width="100%" cellspacing="0" id="tbl_service">
          ...
          <tbody>
            ...
            <?php } ?>
                <tr>
                <td colspan="5"><button type="button" class="btn btn-primary" id="btn_add_service">Add</button></td>
                </tr>
          </tbody>
        </table>
      </div>
    ...
    <div class="card-body">
        ...
        <table class="table table-bordered" width="100%" cellspacing="0" id="tbl_medicine">
          ...
          <tbody>
            ...
            <?php } ?>
                <tr>
                <td colspan="5"><button type="button" class="btn btn-primary" id="btn_add_medicine">Add</button></td>
                </tr>
          </tbody>
        </table>
      </div>
    ...
    <div class="card-body">
        ...
        <table class="table table-bordered" width="100%" cellspacing="0" id="tbl_equipment">
          ...
          <tbody>
            ...
            <?php } ?>
                <tr>
                <td colspan="5"><button type="button" class="btn btn-primary" id="btn_add_equipment">Add</button></td>
                </tr>
          </tbody>
        </table>
      </div>
...
<div class="card shadow mb-4">
    ...
    <div class="card-body">
        ...
        <table class="table table-bordered" width="100%" cellspacing="0">
          ...
          <tbody>
            <?php foreach (get_all("select * from tbl_appointment_status_history where appointment_id = $id") as $row) { ?>
              <tr>
            ...
            <?php } ?>
                            ...
                            <div class="d-flex">
                                <input type="submit" name="update_appointment" class="btn btn-primary" value="Update" <?= in_array($appointment->status,['rejected','closed','completed']) ? 'disabled' : '' ?>>
...
btn_add_medicine.addEventListener('click', function () {  
    ...
    let index = xTable.rows.length - 1;
    let tr = xTable.insertRow(index);
    tr.innerHTML = "<td><select name='mmedicine[]' class='form-control'><?= $medicine_options ?></select></td> <td><input type='number' name='mqty[]' class='form-control' value = '1'></td> </td><td style='text-align: right;' ><?= reset($medicine_array)?></td><td><button type='button' class='btn btn-primary btn-remove'>Remove</button></td>" ;
});
...
function update_service_price(ev){
    if((ev.target.tagName == 'INPUT' && ev.target.type =='number') || ev.target.tagName == 'SELECT'){
        if(ev.target.parentElement.parentElement.children[2].children[0].value == 0 || ev.target.parentElement.parentElement.children[2].children[0].value == ''){
            ...
            ev.target.parentElement.parentElement.children[3].innerHTML = total.toLocaleString("en-US",{ style: 'decimal', minimumFractionDigits: 2,maximumFractionDigits: 2,});
        }
    }
}
...
function update_medicine_price(ev){
    if((ev.target.tagName == 'INPUT' && ev.target.type =='number') || ev.target.tagName == 'SELECT'){
        if(ev.target.parentElement.parentElement.children[1].children[0].value == 0 || ev.target.parentElement.parentElement.children[1].children[0].value == ''){
            ...
            ev.target.parentElement.parentElement.children[2].innerHTML = total.toLocaleString("en-US",{ style: 'decimal', minimumFractionDigits: 2,maximumFractionDigits: 2,});
        }
    }
}
