...
<?php 
$tmp = [];
foreach (get_all("select UPPER(CONCAT(a.status,' - ' ,IFNULL(p.fname,''),' ',IFNULL(p.mname, ''),' ',IFNULL(p.lname,''))) as title,a.appointment_date as start,concat('edit_appointment.php?id=',a.id) as url from tbl_appointment a left join patient p on p.id = a.patient_id where a.patient_id = ".$_SESSION['user']->id."") as $res) {
    $tmp[] = $res;
};
 ?>
    <script>
